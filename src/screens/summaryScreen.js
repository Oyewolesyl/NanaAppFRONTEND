...
import { appState, getActiveChild, savePainLog } from '../appState';
...
export function renderSummaryScreen(app) {
  ...
  screen.querySelector('.pain-type-next-btn').addEventListener('click', () => { const log = savePainLog(); sessionStorage.setItem('nana_last_log_id', log.id); window.location.hash = '#confirmation'; });
  app.append(screen);
}
