...
export function renderConfirmationScreen(app) {
  ...
  screen.insertAdjacentHTML('beforeend', `
    ...
    <section class="summary-card"><p><strong>Time:</strong> ${log ? new Date(log.created_at).toLocaleString() : new Date().toLocaleString()}</p><p><strong>Pain:</strong> ${log?.intensity ?? '-'} / 10</p><p><strong>Spots:</strong> ${formatZones(log?.zones || [])}</p>${log?.notes ? `<p><strong>Note:</strong> ${log.notes}</p>` : ''}</section>
