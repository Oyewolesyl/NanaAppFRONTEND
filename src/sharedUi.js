  ...
  return `
    <article class="child-card ${compact ? 'child-card--compact' : ''}" data-child-id="${child.id}">
      ...
      <div class="child-card-actions">
        ...
        <button type="button" class="child-edit-btn" data-edit-child>
          Edit Profile
        </button>
      </div>
    </article>
...
export function wireChildCards(root, overlay, afterEdit) {
  root.querySelectorAll('[data-child-id]').forEach((card) => {
    ...
    card.querySelector('[data-edit-child]')?.addEventListener('click', () => {
      const child = appState.children.find(c => c.id === card.dataset.childId);
      if (!child || !overlay?.openForChild) return;
      overlay.openForChild(child);
      overlay.afterSave = afterEdit;
    });
  });
}

export function wireAddChildOverlay(root, overlay, afterSave) {
  root.querySelectorAll('.children-add-button,.floating-add-btn,[data-add-child]').forEach((btn) => {
    btn.addEventListener('click', () => {
      overlay.resetForm?.();
      overlay.afterSave = afterSave;
      overlay.hidden = false;
    });
  });
  ...
  overlay.querySelector('.save-child-button')?.addEventListener('click', () => {
    const form = overlay.getFormData?.() || {};
    const fallbackName = form.id ? 'Child' : `Child ${appState.children.length + 1}`;
    const saved = saveChild({
      ...form,
      name: form.name || fallbackName,
      photo_url: form.photo_url || (form.id ? undefined : (appState.children.length % 2 ? ASSETS.secondChildPhoto : ASSETS.childPhoto)),
    });

    overlay.hidden = true;
    if (typeof overlay.afterSave === 'function') overlay.afterSave(saved);
    else if (typeof afterSave === 'function') afterSave(saved);
    else window.location.hash = '#child-added';
  });
}
